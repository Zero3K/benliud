...
//新特征改造，缓存固定以128K为粒度，而不以片的长度做粒度
//这样缓存的调度在对大片长的块调度时有效
//假设有10M缓存，则可以存放80个块
//块采用先进先出的算法排成队列
//从队头挤出数据块，从队尾压入数据块，当有某个数据块被访问时，掉入队尾
#define BUFFER_BLOCK_SIZE (128*1024)

struct TBufferBlock
{
	unsigned int pIndex;	//片索引号
	unsigned int offset;	//片内偏移
	std::string  data;		//长度=BUFFER_BLOCK_SIZE的数据
};  //长度总是BUFFER_BLOCK_SIZE
...
private:
	...
	CBTListener*	m_pSingleListener;	//单端口监听设备地址，全局限速也在其中
    ...
    TPieceMap			m_WriteCache;	//用VECTOR就可以，要排序
	...
	CPeerCenter			m_PeerCenter;		//伙伴管理中心
	...
	_JOB_STATUS m_JobStatus; //任务状态
	//在构造这个表时就把虚文件对片的影响计算出来，以后查表即可
	//前者是片的编号，后者是片内影响范围
